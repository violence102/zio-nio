...
import java.nio.file.{CopyOption, DirectoryStream, FileStore, FileVisitOption, LinkOption, OpenOption, Files => JFiles, Path => JPath}
...
object Files {
  ...
  def createTempFileIn(dir: Path, suffix: String = ".tmp", prefix: Option[String], fileAttributes: Iterable[FileAttribute[_]]): ZIO[Blocking, Exception, Path] =
    effectBlocking(Path.fromJava(JFiles.createTempFile(dir.javaPath, prefix.orNull, suffix, fileAttributes.toSeq: _*))).refineToOrDie[Exception]

  def createTempFile(suffix: String = ".tmp", prefix: Option[String], fileAttributes: Iterable[FileAttribute[_]]): ZIO[Blocking, Exception, Path] =
    effectBlocking(Path.fromJava(JFiles.createTempFile(prefix.orNull, suffix, fileAttributes.toSeq: _*))).refineToOrDie[Exception]

  def createTempDirectory(dir: Path, prefix: Option[String], fileAttributes: Iterable[FileAttribute[_]]): ZIO[Blocking, Exception, Path] =
    effectBlocking(Path.fromJava(JFiles.createTempDirectory(dir.javaPath, prefix.orNull, fileAttributes.toSeq: _*))).refineToOrDie[Exception]

  def createTempDirectory(prefix: Option[String], fileAttributes: Iterable[FileAttribute[_]]): ZIO[Blocking, Exception, Path] =
    effectBlocking(Path.fromJava(JFiles.createTempDirectory(prefix.orNull, fileAttributes.toSeq: _*))).refineToOrDie[Exception]

  def createSymbolicLink(link: Path, target: Path, fileAttributes: FileAttribute[_]*): ZIO[Blocking, Exception, Unit] =
    effectBlocking(JFiles.createSymbolicLink(link.javaPath, target.javaPath, fileAttributes: _*)).unit.refineToOrDie[Exception]
  ...
  def delete(path: Path): ZIO[Blocking, IOException, Unit] = effectBlocking(JFiles.delete(path.javaPath)).refineToOrDie[IOException]
  ...
  def copy(source: Path, target: Path, copyOptions: CopyOption*): ZIO[Blocking, Exception, Unit] =
    effectBlocking(JFiles.copy(source.javaPath, target.javaPath, copyOptions: _*))
      .unit
  ...
  def useFileAttributeView[A <: FileAttributeView: ClassTag, B](path: Path, linkOptions: LinkOption*)(f: A => ZIO[Blocking, Exception, B]): ZIO[Blocking, Exception, B] = {
    val viewClass = classTag[A].runtimeClass.asInstanceOf[Class[A]]  // safe? because we know A is a subtype of FileAttributeView
    effectBlocking(JFiles.getFileAttributeView[A](path.javaPath, viewClass, linkOptions: _*))
      .orDie
      .flatMap(f)
  }

  def readAttributes[A <: BasicFileAttributes: ClassTag](path: Path, linkOptions: LinkOption*): ZIO[Blocking, Exception, A] = {
  ...
  def setAttribute(path: Path, attribute: Attribute, value: Object, linkOptions: LinkOption*): ZIO[Blocking, Exception, Unit] =
    effectBlocking(JFiles.setAttribute(path.javaPath, attribute.toJava, value, linkOptions: _*)).unit.refineToOrDie[Exception]
  ...
  def readAttributes(path: Path, attributes: Attributes, linkOptions: LinkOption*): ZIO[Blocking, Exception, Map[String, AnyRef]] =
    effectBlocking(JFiles.readAttributes(path.javaPath, attributes.toJava, linkOptions: _*))
      .map(_.asScala.toMap)
      .refineToOrDie[Exception]

  def getPosixFilePermissions(path: Path, linkOptions: LinkOption*): ZIO[Blocking, Exception, Set[PosixFilePermission]] =
  ...
  def setPosixFilePermissions(path: Path, permissions: Set[PosixFilePermission]): ZIO[Blocking, Exception, Unit] =
    effectBlocking(JFiles.setPosixFilePermissions(path.javaPath, permissions.asJava))
      .unit
      .refineToOrDie[Exception]
  ...
  def writeLines(path: Path, lines: Iterable[CharSequence], charset: Charset = StandardCharsets.UTF_8, openOptions: Set[OpenOption] = Set.empty): ZIO[Blocking, Exception, Unit] =
    effectBlocking(JFiles.write(path.javaPath, lines.asJava, charset, openOptions.toSeq: _*))
    .unit
    .refineToOrDie[Exception]

  def list(path: Path): ZStream[Blocking, Exception, Path] =
    ZStream.fromEffect(effectBlocking(JFiles.list(path.javaPath).iterator()).refineToOrDie[IOException])
      .flatMap(fromJavaIterator)
      .map(Path.fromJava)

  def walk(path: Path, maxDepth: Int = Int.MaxValue, visitOptions: Set[FileVisitOption] = Set.empty): ZStream[Blocking, Exception, Path] =
    ZStream.fromEffect(effectBlocking(JFiles.walk(path.javaPath, maxDepth, visitOptions.toSeq: _*).iterator()).refineToOrDie[IOException])
      .flatMap(fromJavaIterator)
      .map(Path.fromJava)

  def find(path: Path, maxDepth: Int = Int.MaxValue, visitOptions: Set[FileVisitOption] = Set.empty)
          (test: (Path, BasicFileAttributes) => Boolean): ZStream[Blocking, Exception, Path] = {
    val matcher: BiPredicate[JPath, BasicFileAttributes] = (path, attr) => test(Path.fromJava(path), attr)
    ZStream.fromEffect(effectBlocking(JFiles.find(path.javaPath, maxDepth, matcher, visitOptions.toSeq: _*).iterator()).refineToOrDie[IOException])
      .flatMap(fromJavaIterator)
      .map(Path.fromJava)
  }
